...
type Screen = "list" | "edit" | "preview" | "complete";
...
export function AgreementWorkspace() {
  ...
  return <main className="app-shell">
    ...
    <div className="prototype-note"><Icon name="shield" /><span><strong>画面確認用の試作です。</strong> 外部送信・本番URL・法的効力の保証は行いません。</span></div>

    {screen === "list" && <section className="list-screen">
      <div className="page-heading">
        <div><span className="eyebrow">AGREEMENT PROJECTS</span><h1>仕事の確認事項</h1><p>案件を探し、作成から同意までの状態を確認できます。</p></div>
    ...
    {screen !== "list" && <div className="detail-shell">
      ...
      {screen === "edit" && <section className="editor-screen">
        ...
        <div className="step-nav">{steps.map((label, index) => <button key={label} className={step === index ? "active" : ""} onClick={() => setStep(index)}><span>{index + 1}</span><small>{label}</small></button>)}</div>
