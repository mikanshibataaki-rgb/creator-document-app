...
const steps = ["基本情報", "仕事の内容", "報酬と支払い", "納品と修正", "使用範囲と権利", "キャンセル・追加対応", "契約書プレビュー"];
const betaNotice = "β版です。本サービスは契約書作成を支援するツールであり、法的効力や個別案件への適合性を保証するものではありません。重要な契約は専門家へご確認ください。入力内容はこの端末のブラウザに保存されます。";
...
const stepFields: (keyof AgreementContent)[][] = [
  ...
  ["fee", "taxTreatment", "paymentDue", "paymentMethod"],
  ["deliveryDate", "deliveryPlace", "inspectionDays", "revisionCount", "additionalRevisionFee"],
  ["copyright", "usageScope", "portfolioPermission"],
...
function getChecks(content: AgreementContent): Check[] {
  ...
  if (!content.workDescription) add("law", "依頼する仕事の内容を設定してください", "給付内容・役務内容を分かりやすく記録します。", "workDescription", 1);
  if (!content.fee || Number(content.fee) <= 0) add("law", "報酬を確認してください", "発注者の明示事項として設定を勧めます。", "fee", 2);
  if (!content.deliveryDate) add("law", "納期を設定してください", "給付・役務提供の日として設定を勧めます。", "deliveryDate", 3);
  if (!content.deliveryPlace) add("law", "納品・作業場所を設定してください", "給付・役務提供の場所として設定を勧めます。", "deliveryPlace", 3);
  ...
  if (!content.inspectionDays) add("law", "検査・確認の期限を設定してください", "納品後にいつまで確認するかを整理します。", "inspectionDays", 3);
  if (!content.revisionCount) add("recommended", "無料修正回数を決めましょう", "追加修正・追加依頼の認識違いを防ぎやすくなります。", "revisionCount", 3);
  if (!content.copyright) add("recommended", "著作権の取り扱いを確認しましょう", "制作者に残すか、譲渡するか、使用許諾にするかを整理します。", "copyright", 4);
  if (!content.portfolioPermission) add("recommended", "SNS掲載・実績公開の可否を確認しましょう", "ポートフォリオやSNSへの掲載可否を明確にします。", "portfolioPermission", 4);
  if (!content.cancellation) add("recommended", "キャンセル時の扱いを決めましょう", "準備後のキャンセル料や延期時の対応を整理します。", "cancellation", 5);
  if (!content.reEditingFee) add("recommended", "追加修正・再編集料金を確認しましょう", "納品後に内容を作り直す場合の扱いを整理します。", "reEditingFee", 5);
  return checks;
}
...
export function AgreementWorkspace() {
  ...
  const addProject = () => {
    ...
    setDatabase({ ...database, projects: [project, ...database.projects], agreementVersions: [version, ...database.agreementVersions], timelineEvents: [...database.timelineEvents, createEvent(projectId, versionId, "project_created", "契約書の下書きを作成しました", "sender")] });
  ...
  return <main className="app-shell" data-theme={theme}>
    <header className="app-header">
      <button className="brand-button" onClick={() => setScreen("list")}>
        ...
        <span><b>Creator Agreement</b><small>口約束ではなく、契約書。だけど、お互いにかんたんに。</small></span>
      </button>
    ...
    <div className="prototype-note"><Icon name="shield" /><span>{betaNotice}</span></div>

    {screen === "list" && <section className="list-screen">
      <div className="page-heading">
        <div><span className="eyebrow">CREATOR CONTRACT β</span><h1>口約束ではなく、契約書。<br />だけど、お互いにかんたんに。</h1><p>報酬・納期・修正回数・使用範囲・キャンセル条件を整理し、クリエイターと発注者が安心して確認できる契約書を作成できます。</p></div>
        <button className="primary-button" onClick={addProject}><Icon name="plus" />契約書を作成する</button>
      </div>
      <div className="brand-overview">
        <div><span>01</span><b>仕事の条件を入力</b><small>報酬、納期、作業範囲、納品物、修正回数など、契約前に決めるべき内容を整理します。</small></div>
        <div><span>02</span><b>契約書を作成</b><small>入力した内容から、クリエイター向けの契約書・発注確認書のたたき台を作成します。</small></div>
        <div><span>03</span><b>PDFで確認・保存</b><small>発注者と受注者が同じ内容を確認し、PDFとして保存・共有できます。</small></div>
      </div>
      <section className="capability-card">
        <span className="eyebrow">このアプリでできること</span>
        <ul>
          <li>報酬、納期、納品物、修正回数を整理できます</li>
          <li>著作権、使用範囲、SNS掲載、二次利用の確認ができます</li>
          <li>キャンセル料、追加修正、納期遅延時の扱いを事前に確認できます</li>
          <li>入力内容から契約書・発注確認書のたたき台を作成できます</li>
          <li>PDFとして保存し、発注者・受注者の認識合わせに使えます</li>
          <li>過去の案件を複製して、次の案件にも再利用できます</li>
        </ul>
      </section>
    ...
    {(screen === "edit" || screen === "preview" || screen === "complete") && <div className="detail-shell">
      <aside className="detail-sidebar">
        ...
        <nav>
          <button className={screen === "edit" ? "active" : ""} onClick={() => setScreen("edit")}>契約書を作成</button>
          <button className={screen === "preview" ? "active" : ""} onClick={() => setScreen("preview")}>契約書プレビュー</button>
          {signature && <button className={screen === "complete" ? "active" : ""} onClick={() => setScreen("complete")}>双方確認完了</button>}
        ...
        <div className="side-actions">
          ...
          <button onClick={() => duplicateProject(activeProject)}><Icon name="copy" />この契約書を複製</button>
          <button onClick={downloadPdf}><Icon name="download" />契約書PDFを保存</button>
      ...
      {screen === "edit" && <section className="editor-screen">
        <div className="page-heading compact-heading">
          <div><span className="eyebrow">CONTRACT DRAFT</span><h1>契約書を作成する</h1><p>{locked ? "確定済みの内容は編集できません。修正は新しいバージョンを作成してください。" : "仕事を始める前に、報酬・納期・納品物・修正回数・使用範囲・キャンセル条件を整理します。"}</p></div>
        ...
        <fieldset disabled={locked} className="form-card">
          ...
          {step === 1 && <><div className="section-title"><span>02</span><div><h2>依頼する仕事の内容</h2><p>どんな仕事を、どこまで担当するかを分かる言葉で整理します。</p></div></div><div className="form-grid">
            <Field label="依頼する仕事の内容" wide><textarea name="workDescription" rows={4} value={content.workDescription} onChange={(event) => updateContent("workDescription", event.target.value)} /></Field>
            <Field label="納品するもの" wide><input value={content.deliverables} onChange={(event) => updateContent("deliverables", event.target.value)} /></Field>
          ...
          {step === 2 && <><div className="section-title"><span>03</span><div><h2>報酬と支払い</h2><p>報酬額、税の扱い、支払い方法・支払日を明確にします。</p></div></div><div className="form-grid">
            <Field label="報酬"><input name="fee" inputMode="numeric" value={content.fee} onChange={(event) => updateContent("fee", event.target.value.replace(/\D/g, ""))} /></Field>
            ...
            <Field label="支払い方法"><input name="paymentMethod" value={content.paymentMethod} onChange={(event) => updateContent("paymentMethod", event.target.value)} /></Field>
          </div></>}
          {step === 3 && <><div className="section-title"><span>04</span><div><h2>納品と修正</h2><p>納期、納品場所、確認期限、無料修正回数、追加修正の料金を整理します。</p></div></div><div className="form-grid">
            ...
            <Field label="追加修正の料金" wide><input value={content.additionalRevisionFee} onChange={(event) => updateContent("additionalRevisionFee", event.target.value)} placeholder="例：1回 20,000円〜" /></Field>
          </div></>}
          {step === 4 && <><div className="section-title"><span>05</span><div><h2>使用範囲と権利</h2><p>著作権、使用範囲、二次利用、SNS掲載、実績公開の扱いを整理します。</p></div></div><div className="form-grid">
            ...
            <Field label="使用範囲・二次利用" wide><input value={content.usageScope} onChange={(event) => updateContent("usageScope", event.target.value)} />
            </Field>
          ...
          {step === 5 && <><div className="section-title"><span>06</span><div><h2>キャンセル・追加対応</h2><p>キャンセル時の扱い、追加修正、納期遅延時、素材や撮影に関する万一の取り決めを整理します。</p></div></div><div className="clause-list">
            ...
            <Field label="追加修正・再編集料金" wide><input name="reEditingFee" value={content.reEditingFee} onChange={(event) => updateContent("reEditingFee", event.target.value)} placeholder="例：内容確認後に別途見積" /></Field>
            <Field label="キャンセル時の扱い" wide><textarea name="cancellation" rows={3} value={content.cancellation} onChange={(event) => updateContent("cancellation", event.target.value)} /></Field>
          ...
          {step === 6 && <div className="draft-preview-panel"><div className="section-title"><span>07</span><div><h2>契約書プレビュー</h2><p>入力内容をもとに、契約書・発注確認書のたたき台を表示します。内容を確認してからPDF保存してください。</p></div></div><div className="preview-action-card"><p>{betaNotice}</p><button type="button" className="primary-button" onClick={() => setScreen("preview")}>契約書プレビューを開く</button><button type="button" className="secondary-button" onClick={downloadPdf}>PDFで保存する</button></div></div>}
        </fieldset>
        <aside className="right-assist-card">
          <span className="eyebrow">入力補助</span>
          <h3>契約前に確認したい項目</h3>
          <p>報酬、納期、修正回数、使用範囲、キャンセル条件が入っているかを確認します。</p>
          <div className="mini-check-list">{checks.slice(0, 5).map((check) => <button type="button" key={`${check.field}-${check.title}`} onClick={() => setStep(check.step)}><span className={`level-dot ${check.level}`} /><span>{check.title}</span></button>)}{!checks.length && <p className="complete-mini"><Icon name="check" />入力内容が整っています。</p>}</div>
          <div className="mini-paper"><b>契約書プレビュー</b><span>{content.projectName || "案件名未設定"}</span><small>報酬 {yen(content.fee)} ／ 納期 {formatDate(content.deliveryDate)}</small></div>
        </aside>
        </div>
        <div className="editor-footer"><button className="ghost-button" disabled={step === 0} onClick={() => setStep(Math.max(0, step - 1))}>← 前の項目へ戻る</button>{step < 6 ? <button className="primary-button" onClick={() => setStep(step + 1)}>次の項目へ進む <Icon name="arrow" /></button> : <button className="primary-button" onClick={() => setScreen("preview")}>契約書を作成する <Icon name="arrow" /></button>}</div>
      </section>}
        ...
        <div className="preview-grid">
          ...
          <article className="agreement-card">
            ...
            <div className="agreement-body">
              ...
              <section className="summary-section"><span>01</span><div><h2>依頼する仕事の内容</h2><p>{content.workDescription || "未設定"}</p></div></section>
              ...
              <section className="detail-section"><h2>納品と修正</h2><dl><div><dt>無料修正</dt><dd>{content.revisionCount ? `${content.revisionCount}回まで` : "未設定"}</dd></div><div><dt>追加修正</dt><dd>{content.additionalRevisionFee || "未設定"}</dd></div></dl></section>
              <section className="detail-section"><h2>使用範囲と権利</h2><dl><div><dt>著作権 <HelpButton id="copyright" onOpen={setHelpId} /></dt><dd>{content.copyright || "未設定"}</dd></div><div><dt>使用範囲・二次利用</dt><dd>{content.usageScope || "未設定"}</dd></div><div><dt>SNS掲載・実績公開</dt><dd>{content.portfolioPermission === "可" ? `可 / ${content.portfolioTiming} / ${content.creditRequired}` : content.portfolioPermission || "未確認"}</dd></div></dl></section>
      ...
      {screen === "complete" && <section className="complete-screen">
        <div className="complete-card"><div className="complete-icon"><Icon name="check" /></div><span>契約内容を確認しました</span><h1>双方で同じ内容を確認しました</h1><p>送信者が作成したVer.{activeProject.versionNumber}の契約書・発注確認書を、{signature?.signerName}さんが確認しました。</p><div className="complete-meta"><div><span>契約番号</span><b>{activeProject.projectNumber}</b></div><div><span>確認日時</span><b>{formatDateTime(signature?.agreedAt ?? "")}</b></div><div><span>メール確認</span><b>{recipient?.verificationStatus ?? "未確認"}</b></div></div><button className="primary-button large-button" onClick={downloadPdf}><Icon name="download" />契約書PDFを保存</button><small>PDF保存後も、内容に不明点がある場合は双方で確認してください。重要な契約では専門家への確認をおすすめします。</small></div>
      </section>}
    </div>}
